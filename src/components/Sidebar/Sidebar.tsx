import {
    MenuFoldOutlined,
    MenuUnfoldOutlined,
    HeartFilled,
    CalendarTwoTone,
    IdcardOutlined,
    TrophyFilled,
} from '@ant-design/icons';
import { Layout, Menu, Button } from 'antd';
import React, { useState } from 'react';
import './Sidebar.scss';
import { useMediaQuery } from '../../hooks/breackpoints';
import Icon from '@ant-design/icons/lib/components/Icon';
const { Sider } = Layout;
const exitSvg = () => (
    <svg width='15' height='16' viewBox='0 0 15 16' fill='none' xmlns='http://www.w3.org/2000/svg'>
        <path
            d='M3.74621 7.39397V5.86897C3.74621 5.80112 3.66943 5.76183 3.61585 5.80469L0.919425 7.93683C0.90984 7.94439 0.902093 7.95402 0.896766 7.965C0.891439 7.97598 0.888672 7.98802 0.888672 8.00022C0.888672 8.01243 0.891439 8.02447 0.896766 8.03545C0.902093 8.04643 0.90984 8.05606 0.919425 8.06362L3.61585 10.1975C3.66764 10.2386 3.74621 10.2011 3.74621 10.1333V8.60826H10.6664V7.39397H3.74621Z'
            fill='black'
        />
        <path
            fillRule='evenodd'
            clipRule='evenodd'
            d='M4.62716 0.929688H14.0474C14.3242 0.929688 14.5474 1.1529 14.5474 1.42969V14.5725C14.5474 14.8493 14.3242 15.0725 14.0474 15.0725H4.62716C4.35038 15.0725 4.12716 14.8493 4.12716 14.5725V11.5725C4.12716 11.5333 4.15931 11.5011 4.19859 11.5011H5.27002C5.30931 11.5011 5.34145 11.5333 5.34145 11.5725V13.8583H13.3331V8.60826V7.39397V2.14397H5.34145V4.42969C5.34145 4.46897 5.30931 4.50112 5.27002 4.50112H4.19859C4.15931 4.50112 4.12716 4.46897 4.12716 4.42969V1.42969C4.12716 1.1529 4.35038 0.929688 4.62716 0.929688Z'
            fill='black'
        />
    </svg>
);
const ExitIcon = () => <Icon component={exitSvg} />;

function Sidebar() {
    const [collapsed, setCollapsed] = useState(false);
    const data = useMediaQuery('(max-width: 833px)');

    return (
        <div className='v-menu'>
            <Sider
                theme='light'
                width={data.siderWidth}
                className='sider-bar'
                trigger={null}
                collapsedWidth={data.siderCollapsedWidth}
                collapsible={true}
                collapsed={collapsed}
            >
                <div>
                    <div className='sider-bar__logo'>
                        {!collapsed && <img src={data.openedLogo} alt='logo' />}
                        {collapsed && <img src={data.closedlogo} alt='logo' />}
                    </div>
                    <Menu
                        className='sider-bar__menu'
                        style={{ padding: 0 }}
                        defaultSelectedKeys={['']}
                        items={[
                            {
                                key: '1',
                                label: 'Календарь',
                                icon: !data.isMobile && <CalendarTwoTone />,
                            },
                            {
                                key: '2',
                                label: 'Тренировки',
                                icon: !data.isMobile && <HeartFilled />,
                            },
                            {
                                key: '3',
                                label: 'Достижения',
                                icon: !data.isMobile && <TrophyFilled />,
                            },
                            {
                                key: '4',
                                label: 'Профиль',
                                icon: !data.isMobile && <IdcardOutlined />,
                            },
                        ]}
                    />
                </div>
                <div className='sider-bar__swither'>
                    <Button
                        type='text'
                        icon={!data.isMobile && <ExitIcon></ExitIcon>}
                        onClick={() => setCollapsed(!collapsed)}
                        block
                    >
                        {!collapsed && <>Выход</>}
                    </Button>
                </div>
            </Sider>
            <div className='sider-switcher' style={{ left: collapsed ? '64px' : '208px' }}>
                <div data-test-id={data.isMobile ? 'sider-switch-mobile' : 'sider-switch'}>
                    {React.createElement(collapsed ? MenuUnfoldOutlined : MenuFoldOutlined, {
                        className: 'trigger',
                        onClick: () => {
                            return setCollapsed(!collapsed);
                        },
                    })}
                </div>
            </div>
        </div>
    );
}

export default Sidebar;
